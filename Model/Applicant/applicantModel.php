<?php
	...
	function applicantLogin($userName, $password){
		$conn = getConnection();
		$sql = "select * from applicant_info where username='{$userName}' and password='{$password}'";
  ...
  function applicantProfileShow($userName){
	...
	function applicantRegistration($user){
		$conn = getConnection();
		$sql = "insert into applicant_info values('', '{$user['name']}', '{$user['email']}', '{$user['password']}',  '{$user['dateOfBarth']}','{$user['userName']}', '{$user['occupation']}', '{$user['yearlyIncome']}', '{$user['gender']}')";
